...
for (const studentData of studentsStr.split("; ")){
    const [student, group] = studentData.split(" - ");
...
Object.values(studentsGroups)
    .forEach((studentsArray: string[]) => studentsArray.sort());
...
class CoordinateOK {
    ...
    constructor(direction: Direction, ...coordinate: any) {
        ...
        this._degrees = this._direction === Direction.latitude ?
            CoordinateOK._validate(degrees , this.MIN_LAT_DEGREES, this.MAX_LAT_DEGREES) :
            CoordinateOK._validate(degrees , this.MIN_LON_DEGREES, this.MAX_LON_DEGREES);
        this._minutes = CoordinateOK._validate(minutes, this.MIN_MINUTES, this.MAX_MINUTES);
        this._seconds = CoordinateOK._validate(seconds, this.MIN_SECONDS, this.MAX_SECONDS);
        this._zone = CoordinateOK._getZone(direction, degrees);
        this._point = CoordinateOK._calculatePoint(degrees, minutes, seconds);
    }
    ...
    static fromZeros(direction: Direction): CoordinateOK {
        return new CoordinateOK(direction, BigInt(0), BigInt(0), BigInt(0));
    }
    ...
    public midpoint(coordinate: CoordinateOK): CoordinateOK | null {
        ...
        return new CoordinateOK(direction, degrees, minutes, seconds);
    }

    static middlePoint(c1: CoordinateOK, c2: CoordinateOK): CoordinateOK | null {
        return c1.midpoint(c2);
    }
}

const coord1 = new CoordinateOK(Direction.latitude);
const coord2 = new CoordinateOK(Direction.latitude, BigInt(10), BigInt(10), BigInt(10));
...
const coordmid2 = CoordinateOK.middlePoint(coord2, coord1);
